...
class Simple extends IController
{
	...
	function init()
	{
		...
		$user['useremail'] = ISafe::get('email');
	...
	//AJAX校验有效性
    function reg_CheckValidate()
    {
    	$email      = IFilter::act(IReq::get('email','get'));
    	$username   = IFilter::act(IReq::get('username','get'));
		$VCode   = IFilter::act(IReq::get('VCode','get'));
    	$message    = '';
		...
		/*注册信息校验*/
			...
			else if($username != '' && !Util::is_username($username))
			{
		...
		//if(strpos($_SERVER['REQUEST_URI'],'VCode')){
			$captcha = ISafe::get('Captcha');
    ...
    function loginAfter($userRow)
    {
		...
		ISafe::set('useremail',$userRow['email']);
    ...
    function do_find_password()
	{
		...
		if($step==1){
			...
			$user = $tb_user->query("username='{$username}' or email='{$useremail}'");
			if(!$user)
			{
				unset($result['errType']);
				$result['error'] = "账户不存在";
				echo JSON::encode($result);
				return;
			}
			...
			if( $tb_find_password->query("`hash` = '{$hash}'") || $tb_find_password->add()  )
			{
				...
				$url = IUrl::creatUrl("/simple/restore_password/hash/{$hash}/step/3");
	...
	function restore_password()
	{
		...
		if($step==3){
			...
			if(!$row)
			{
				$this->message = "本链接已失效，请重新申请密码找回链接";
				$this->redirect("restore_password");
				return;
			}
	...
	function do_restore_password()
	{
		...
		{
			$result['success'] = -2;
			$result['message'] = "本链接已失效，请重新申请密码找回链接";
			echo JSON::encode($result);
			exit;
		}
		...
		{
			$result['success'] = -2;
			$result['message'] = '新密码至少六位，且两次输入的密码应该一致。';
			echo JSON::encode($result);
			return;
		}
		$pwd = md5($pwd);
