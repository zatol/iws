...
class Site extends IController
{
	...
	function keywords()
	{
		$_hotkeywordCallback = '_hotkeywordCallback({keywords:\'<div class=\"searchWordsAllbody\" id=\"searchWordsAllbody\"><div class=\"searchWordsAbTop\"></div><div class=\"searchWordsAbMid\"><div class=\"searchWordschild\"><div class=\"searchWordschildclass searchWordschildclassH\"><span>热词</span></div><div class=\"searchWordschilddetail\">';
		$hotkeywordCallback_ = '<span class=\"blank0\"></span></div><span class=\"blank0\"></span></div></div><div class=\"searchWordsAbBotm\"></div></div>\'})';
		//$resultArray = array('keywords'=>'');
		$resulthref = '';
		$orderObj   = new IQuery('keyword as kw');
		$orderObj->fields  = 'kw.word';
		$orderObj->where   = 'kw.hot = 1';
		$orderObj->order  = 'kw.order asc';
		$orderObj->limit  = ' 50 ';
		$orderList = $orderObj->find();
		foreach($orderList as $val)
		{
			$resulthref .= '<a class=\"track\" name=\"hp-hp-head-swords_m-n\" href=\"'.Iurl::creatUrl("").'site/search_list/word/'.urlencode($val['word']).'\">'.$val['word'].'</a>';
		}
		//echo $resulthref;
		echo $_hotkeywordCallback.$resulthref.$hotkeywordCallback_;
