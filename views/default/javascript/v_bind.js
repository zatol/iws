...
$(document).ready(function () {
    ...
    $("#gotoReg").click(function () {
        ...
        window.location = "http://" + domain + "simple/reg";;
        return false;
    });
...
function validateVJiaInfo() {
    ...
    if (userName.val().trim() == "" || userName.val().trim() == "Email/手机号") {
        ...
        userNameError.css("visibility", "visible");
        isValid = false;
    } else if (password.val().trim() == "" || !/^[\w-]+(\.[\w-]+)*@[\w-]+(\.(\w)+)*(\.(\w){2,3})$/.test(password.val())) {
        password.focus();
...
function login() {
    ...
    loginButton.html("<img src='views/default/skin/default/images/front/loading.gif'/>正在验证...");
    loginButton.unbind("click");

    var success = false;
    if (window.location.href.toLowerCase().indexOf('demologin') > 0) domain = 'demologin.vancl.com';
    var loginUrl = document.location.protocol + "//" + domain + "simple/bind_exists_user";
    var loginData = "Loginasync=true&login_info=" + encodeURIComponent($("#vanclUserName").val().trim()) + "&password=" + encodeURIComponent($("#vanclPassword").val().trim())
        ...
        data = eval("(" + data + ")");
            if (data != null && data != "" && data.Error != "") {
                ...
                if (data != null && data != "" && data.ErrorType == '5') {
					window.location = getRedirectUrl(data.ErrorType);
                    success = false;
                }
                else {
...
function loginVJiaUser() {
    ...
    if (success) {
        ...
        var vjiaValidating = loginUrl + "simple/bind_nexists_user";
        var b = true;
        $.ajax({
            type: "POST",
            url: vjiaValidating,
            ...
            data: {
                ...
                email: $('#PartnerPassword').val().trim(),
                        ...
                        if (error != "") {
                            ...
                            switch (errorType) {
								case 5:
									...
									window.location = getRedirectUrl(errorType);
                                    break;
                                case 2:
                                    errElement = $("#vjiaValidateError");
                                    break;
