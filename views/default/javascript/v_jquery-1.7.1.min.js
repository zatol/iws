//[/jquery/jquery-1.7.1.min,jquery.fn.alert],[/Login/StringTrim.js],[/public/trace.js]
(function (a, b) {
	function cy(a) {
...
// prefix 80EE8D67985E
$(function () {
    $.fn.alert = function (message, callback, opt) {
        var option = $.extend(false, {}, $.fn.alert.defaults, opt);
        $('.orderChangeTsk80EE8D67985E,#maskdiv80EE8D67985E').remove();

        //#region 创建遮罩层并添加到body
        var maskdiv = '<div id="maskdiv80EE8D67985E"></div>';
        $(maskdiv).appendTo($('body'));
        //#endregion

        //#region 创建alert代码并添加到body
        var html = [];
        html.push('<div class="orderChangeTsk80EE8D67985E">');
        html.push('<h3><span>' + option.title + '</span><a href="#" class="close80EE8D67985E">关闭</a>');
        html.push('<a class="close80EE8D67985E" href="#"><img src="//i.vanclimg.com/Others/2011/2/15/dpbox_06.gif"></a></h3>');
        html.push('<p class="Word80EE8D67985E" style="text-align:center;">' + message + '</p>');
        html.push('<div id="buttons80EE8D67985E"><input type="submit" value="" class="queding80EE8D67985E"><span><a id="quxiao80EE8D67985E" href="#">取消</a></span></div></div>');
        $(html.join('')).appendTo($('body'));
        //#endregion

        //#region 根据类型调整样式 1个按钮 2个按钮
        if (option.type == 1) {
    ...
    //#region alert插件默认属性 该处设置了标题及类型
    $.fn.alert.defaults = {
        title: '提示',
        type: 1 // 1： 为只有确定按钮 2： 确定 取消两个按钮
    };
    //#endregion

    //#region 将元素设置到指定的位置 css({left, top}) 
    $.fn.center = function () {
        var t = (($(window).height() - this.height()) / 2 - 80).toString() + "px";
        var l = (($(window).width() - this.width()) / 2).toString() + "px";
        if ($.browser.msie && $.browser.version == '6.0') {
            t = ((($(window).height() - this.height()) / 2 - 80) + $(window).scrollTop()).toString()
        }
        return this.css({ top: t, left: l }).show();
    };
    //#endregion
});
...
VA_GLOBAL.va = {
    ...
    start: function() {
        try {
            if (typeof VA_GLOBAL.start != "undefined") return;
            VA_GLOBAL.start = "1";

            //alert("trace.js...");
            var begintime = new Date().getTime();
            VA_GLOBAL.begintime = begintime;
            VA_GLOBAL.pvid = VA_GLOBAL.Lang.timeSeq32();

            var href = window.location.href;
            var hash = window.location.hash;
            //if (hash.length > 0) href = href.substring(0, href.length-hash.length);
            var idxHD1 = href.indexOf("//", 0);
            var idxHD2 = href.indexOf("/", idxHD1 + 2);
            VA_GLOBAL.urlHost = href.substring(idxHD1, idxHD2 > 0 ? idxHD2 : href.length); //exclude http: or https:

            //var hrefencoded = encodeURIComponent(href);
            var protocol = window.location.protocol.toLowerCase();
            var desthost = "//vamt.vancl.com:";
            //var desthost = "//10.251.16.5:";
            var destport = protocol == "https:" ? 443 : 80;
            VA_GLOBAL.server = protocol + desthost + destport;
            //var domain = document.domain;
            var domain = window.location.hostname.toLowerCase();
            var isip = VA_GLOBAL.Http.isIp(domain);
            var idx = domain.lastIndexOf(".");
            if (idx > 0) idx = domain.lastIndexOf(".", idx - 1);
            var domain1 = isip ? domain : (idx == -1 ? ("." + domain) : domain.substring(idx)); //.vancl.com
            VA_GLOBAL.domain1 = domain1;
            //alert(VA_GLOBAL.domain1);
            var s_ip = "-"; //domain;
            var s_port = window.location.port;
            var uri = window.location.pathname;
            if (VA_GLOBAL.Lang.isEmpty(uri)) uri = "/";
            ...
            if (VA_GLOBAL.Lang.isNone(src)) {
                src = null;
            }

            //cookies
            //track info
            var trackinfo = VA_GLOBAL.SubCookie.getAll("va_track"); //ppvid,tid,url,tr,lb
            var tr_ppvid = "-";
            var tr_tid = "-";
            var tr_val = "-";
            var tr_label = "-";
            var tr_url = (typeof trackinfo["url"] == "undefined") ? "-" : trackinfo["url"];
            var targethref = null;
            if (/^https?:\/\/./i.test(tr_url)) {
                targethref = href;
            }
            else {
                var idxnjk = href.indexOf("//", 0);
                targethref = href.substring(idxnjk);
            }
            //insite search way
            var isw = (typeof trackinfo["extras"] == "undefined") ? "" : trackinfo["extras"];
            //alert(targethref);
            if (decodeURI(tr_url) == decodeURI(targethref)) {
                //alert("matched!");
                tr_ppvid = (typeof trackinfo["ppvid"] == "undefined") ? "-" : trackinfo["ppvid"];
                tr_tid = (typeof trackinfo["tid"] == "undefined") ? "-" : trackinfo["tid"];
                tr_val = (typeof trackinfo["tr"] == "undefined") ? "-" : trackinfo["tr"];
                tr_label = (typeof trackinfo["lb"] == "undefined") ? "-" : trackinfo["lb"];
                VA_GLOBAL.Cookie.unset("va_track", "/", domain1, null);
            }

            var va_vs = null;
            //session info
            var va_opened = VA_GLOBAL.Cookie.get("va_opened");
            //alert(va_opened);
            var va_sid = VA_GLOBAL.Cookie.get("va_sid");
            //var newsession = false;
            if (va_sid == null || va_opened == null) {
                va_sid = VA_GLOBAL.Lang.timeSeq32();
                va_vs = "l";
                VA_GLOBAL.Cookie.set("va_opened", "1", "", "/", domain1, null);
            }
            else {
                va_vs = "g";
            }
            VA_GLOBAL.sid = va_sid;
            var dtse = new Date();
            dtse.setTime(begintime + 30 * 60 * 1000);
            var a30min = dtse;
            VA_GLOBAL.Cookie.set("va_sid", va_sid, a30min, "/", domain1, null);
            //user info
            var userinfo = VA_GLOBAL.SubCookie.getAll("va_user"); //uid,uvc,ft,lt,tt
            var va_uid = userinfo["uid"]; //VA_GLOBAL.Cookie.get("va_uid");
            var va_uvc = userinfo["uvc"];
            if (VA_GLOBAL.Lang.isNone(va_uid)) { //new user
                va_uid = VA_GLOBAL.Lang.timeSeq32();
                va_uvc = 1;
                userinfo["uid"] = va_uid;
                userinfo["uvc"] = va_uvc;
                ...
                try {
                    va_uvc = Number(va_uvc) + 1;
                    if (Number(va_uvc) > 999) va_uvc = 999;
                }
                catch (uvcex) {
                    va_uvc = 1;
                }
            ...
            //src to cookie
            if (src != null) {
                VA_GLOBAL.Cookie.set("va_src", src, "", "/", domain1, null);
            }
            else {
                src = VA_GLOBAL.Cookie.get("va_src");
                if (VA_GLOBAL.Lang.isNone(src)) {
                    src = null;
                }
            }
            VA_GLOBAL.uid = va_uid;
            var dtur = new Date();
            dtur.setTime(begintime + 730 * 24 * 3600 * 1000); //two years
            var a1year = dtur;
            VA_GLOBAL.SubCookie.setAll("va_user", userinfo, a1year, "/", domain1, null);

            //search type
            var iswv = "";
            if ((typeof track_sinput != "undefined") && track_sinput != null && track_sinput != "") {
                iswv = "iswv=" + track_sinput;
            }
            ...
            val += "\t" + va_uvc; //
            val += "\t" + "j"; //x-gm
            val += "\t" + protocol.substring(0, protocol.length - 1) + "/1.1"; //cs-version
            val += "\t" + domain; //cs-host
            val += "\t" + s_ip; //s-ip
            val += "\t" + ((s_port + "") == "" ? "-" : s_port); //s_port
            val += "\t" + uri; //uri
            val += "\t" + (query == "" ? "-" : encodeURIComponent(query).substring(0, 400)); //query string  截断400
            val += "\t" + "-"; //s-sitename
            val += "\t" + "-"; //cs-username
            val += "\t" + "GET"; //cs-method
            val += "\t" + "-"; //sc-status
            ...
            val += "\t" + "-"; //cs-bytes
            val += "\t" + "-"; //time-taken
            val += "\t" + (referrer == "" ? "-" : encodeURIComponent(referrer.replace(/[\r\n\t]/g, " ").substring(0, 400))); //referrer 截断400

            val += "\t" + "va_rt=" + "p"; // x-extras
            if (src != null) val += "&va_src=" + src;
            val += "&va_uft=" + userinfo["ft"];
            val += "&va_ult=" + userinfo["lt"];
            val += "&va_utt=" + userinfo["tt"];
            if (iswv != "") val += "&" + iswv;
            if (isw != "") val += "&" + isw;
            //val += "&va_pid=" + ;
            val += "&va_tid=" + tr_tid;
            val += "&va_track=" + tr_val;
            val += "&va_label=" + tr_label;

            val += "\t" + (ua == "" ? "-" : encodeURIComponent(ua.replace(/[\r\n\t\'\"]/g, " "))); //user agent. ua.replace(/[\r\n\t\'\"]/g,"")  
            //val += "\t" + (cookies == "" ? "-" : cookies.replace(/[\r\n\t]/g," ")); //cookie  //找出有用的

            var sidcookie = VA_GLOBAL.Lang.isNone(VA_GLOBAL.Cookie.get("sid")) ? "00000000000000000000000000000000" : VA_GLOBAL.Cookie.get("sid");
            val += "\t" + (sidcookie == "" ? "-" : "sid=" + sidcookie.replace(/[\r\n\t]/g, " "));
            var str = protocol + desthost + destport + "/mt/visit?ct=js&v=2&info=" + encodeURIComponent(val);
            VA_GLOBAL.PVURL = str;


            //load event
            //VA_GLOBAL.Event.addHandler(window, "load", function(event) {
            //  VA_GLOBAL.va.loaded();
            //});js
    ...
    send: function() {
        try {
            if (typeof VA_GLOBAL.send != "undefined") return;
            VA_GLOBAL.send = "1";

            if (typeof VA_GLOBAL.server != "undefined") {
                //VA_GLOBAL.Dom.loadImageBeacon(VA_GLOBAL.PVURL);
    ...
    loaded: function() {
        try {
            if (typeof VA_GLOBAL.loaded != "undefined") return;
            VA_GLOBAL.loaded = "1";

            //send load time
            if (typeof VA_GLOBAL.server != "undefined") {
                var lt = new Date().getTime() - VA_GLOBAL.begintime;
                var val = VA_GLOBAL.sid + "\t" + VA_GLOBAL.uid + "\t" + VA_GLOBAL.pvid + "\t" + lt;
                //send
                var str = VA_GLOBAL.server + "/mt/render?ct=js&v=2&info=" + encodeURIComponent(val);
                //VA_GLOBAL.Dom.loadScriptURL(str);
                //VA_GLOBAL.Dom.loadImageBeacon(str);
                $.getScript(str);
            }
            ...
            VA_GLOBAL.Event.addHandler(document, "mousedown", function(event) { //click
                ...
                if (event.button == 0 || event.button == 1) {
                    ...
                    if (node.nodeType == 1) {//element
                        var found = VA_GLOBAL.va.trackNode(node);
                        if (found == false) {
                            VA_GLOBAL.va.trackNode(node.parentNode);
                        }
            ...
            /*
            *  //stay time
            *  VA_GLOBAL.Event.addHandler(window, "beforeunload", function(event) {
            *    if (typeof VA_GLOBAL.server != "undefined") {
            *      var lt = new Date().getTime() - begintime;
            *      var val = VA_GLOBAL.pvid + "\t" + lt;
            *      //send
            *      var str = VA_GLOBAL.server + "/mt/stay?ct=js&info=" + encodeURIComponent(val);
            *      VA_GLOBAL.Dom.loadImageBeacon(str);
            *      var bt = new Date().getTime();
            *      while (true) if ((new Date().getTime() - bt) > 100) break;
            *    }
            *  });
            */
    ...
    trackNode: function(node) {
        ...
        var b = false;
        var va_click = node.getAttribute("va_track");
        if ((typeof va_click == "undefined") || va_click == null || va_click == "") {
            ...
            if (b == false || (typeof va_click == "undefined") || va_click == null || va_click == "") va_click = "-";
        ...
        if (b == true || nodeName == "a") {
            //var va_cid = VA_GLOBAL.Lang.timeSeq32();
            //get label
            var label = null;
            try {
                if (nodeName == "a") {
            ...
            if ((typeof label == "undefined") || label == null) { // || label.toString().trim() == ""
                label = "-";
            }
            ...
            if (label.length > 100) label = encodeURIComponent(label.substring(0, 100));
            VA_GLOBAL.va.track(turl, va_click, label);
            b = true;
        } //end if (b == true || nodeName == "a")

        return b;
    },

    track: function(url, value, label) {
        var tid = VA_GLOBAL.Lang.timeSeq32();
        ...
        if ((typeof track_sinput == "undefined") || track_sinput == null || track_sinput == "") {
            ...
            extras = "isw=" + track_sinput;
        }
        //pass track info to next page
        if (url != null && (typeof url != "undefined") && url != "") {
            //url = url.trim();
            var trackinfo = {};
            trackinfo["ppvid"] = VA_GLOBAL.pvid;
            trackinfo["tid"] = tid;
            trackinfo["url"] = url;
            trackinfo["tr"] = value;
            trackinfo["lb"] = label;
            trackinfo["extras"] = extras;
            var dt = new Date();
            dt.setTime(new Date().getTime() + 3 * 60 * 1000);
            var a3min = dt;
            VA_GLOBAL.SubCookie.setAll("va_track", trackinfo, a3min, "/", VA_GLOBAL.domain1, null);
        }
        //else {
        url = extras; //"-";  
        //send
        if (typeof VA_GLOBAL.server != "undefined") {
            var val = VA_GLOBAL.sid + "\t" + VA_GLOBAL.uid + "\t" + VA_GLOBAL.pvid + "\t" + tid + "\t" + url + "\t" + value + "\t" + label;
            var str = VA_GLOBAL.server + "/mt/click?ct=js&v=2&info=" + encodeURIComponent(val); // + "&rnd=" + VA_GLOBAL.Lang.random(1000000000, 9999999999)
            //VA_GLOBAL.Dom.loadScriptURL(str);
            //VA_GLOBAL.Dom.loadImageBeacon(str);
            $.getScript(str);
        }
    ...
    saveInfo: function(type, name, value) {
        var tid = VA_GLOBAL.Lang.timeSeq32();
        ...
        //send
        if (typeof VA_GLOBAL.server != "undefined") {
            var val = VA_GLOBAL.sid + "\t" + VA_GLOBAL.uid + "\t" + VA_GLOBAL.pvid + "\t" + tid + "\t" + type + "\t" + name + "\t" + value;
            var str = VA_GLOBAL.server + "/mt/depot?ct=js&v=2&info=" + encodeURIComponent(val);
            //VA_GLOBAL.Dom.loadScriptURL(str);
            //VA_GLOBAL.Dom.loadImageBeacon(str);
            $.getScript(str);
        }
...
VA_GLOBAL.vanew = {
    ...
    prepare: function() {
        ...
        if (referer == null || (typeof referer == "undefined")
				|| referer == "") {
            VA_GLOBAL.Cookie.unset("va_click", "/", domain1, null);
        ...
        VA_GLOBAL.Cookie.set("va_sid", va_sid, a24h, "/", domain1, null);
        // click
        ...
        VA_GLOBAL.SubCookie.setAll("va_visit", userinfo, a1year, "/", domain1,
				null);
        // track_sinput
        var insitesearchway = "-";
    ...
    elementclicked: function(node) {
        ...
        // is track
        if (istrack) {
            trackname = node.attributes["name"].value;
        }
...
try {
  $(document).ready(
    function() {
      ...
      //原来程序
      /////////////////////////////////////////////////////
      if (typeof VA_GLOBAL.v3start != "undefined") return;
      VA_GLOBAL.v3start = "1";

      var weblog_trace_sb = [];
      var gatherCookies = getPageLab();

      //锚点
      var hash = (window.location.hash.length > 0 ? window.location.hash.substring(1) : "");
           
      weblog_trace_sb.push(document.location.protocol);
      weblog_trace_sb.push("//vamr.vancl.com/trace.ashx?ref=");
      weblog_trace_sb.push(escape(window.document.referrer));
      
      if (typeof (track_sinput) != "undefined") {
        weblog_trace_sb.push("&trace_sib=" + track_sinput);
      }
      
      if (gatherCookies != "") {
        weblog_trace_sb.push("&gather_cookies=" + gatherCookies);
    }
      if (!VA_GLOBAL.Lang.isNone(hash)) {
          weblog_trace_sb.push("&hash=" + hash);
      }
      
      var weblog_trace_url = weblog_trace_sb.join("");
      try {
        /*
         *        var weblog_trace_script = document.createElement('script');
         *        weblog_trace_script.setAttribute('src', weblog_trace_url);
         * 
         *        // load the script
         *        document.getElementsByTagName('head')[0].appendChild(weblog_trace_script);
         */
        $.getScript(weblog_trace_url);
    } catch (e) { }
...
try {
  $(window).load(
    function() {
      ...
      //原来程序
      /////////////////////////////////////////////////////
      if (typeof VA_GLOBAL.v3loaded != "undefined") return;
      VA_GLOBAL.v3loaded = "1";
      
      var weblog_trace_sb = [];
      weblog_trace_sb.push(document.location.protocol);
      weblog_trace_sb.push("//vamr.vancl.com/trace_loadtime.ashx?ref=");
      weblog_trace_sb.push(escape(window.document.referrer));
      
      var rand = Math.floor(Math.random() * 10);
      if (typeof (weblog_loadtime) != "undefined" && rand == 0) {
        var wl_loadtime = (new Date()) - weblog_loadtime;
        weblog_trace_sb.push("&loadtime=" + wl_loadtime);
      } else return;
                 
      var weblog_trace_loadtime_url = weblog_trace_sb.join("");
      try {
        var weblog_trace_script = document.createElement('script');
        weblog_trace_script.setAttribute('src', weblog_trace_loadtime_url);
...
(function (a, b) {
	...
	a.fn.lazyload = function (e) {
		var c = this;
		var f = {threshold:0, failure_limit:0, event:"scroll", effect:"show", container:b, data_attribute:"original", skip_invisible:false, appear:null, load:null, placeholder:"http://i.vanclimg.com/search/color3.gif"};
		a.extend(f, e);
		function g() {
			var h = 0;
			c.each(function () {
				...
				if (a.abovethetop(this, f) || a.leftofbegin(this, f)) {
					...
					if (!a.belowthefold(this, f) && !a.rightoffold(this, f)) {
						i.trigger("appear");
					} else {
					}
				}
		...
		if (e) {
			...
			if (undefined !== e.effectspeed) {
				e.effect_speed = e.effectspeed;
				delete e.effectspeed;
			}
			a.extend(f, e);
		}
		$container = (f.container === undefined || f.container === b) ? $window : a(f.container);
		if (0 === f.event.indexOf("scroll")) {
			$container.bind(f.event, function (h) {
				return g();
			});
		}
		var d = new Array();
		this.each(function () {
			...
			var h = a(i);
			i.loaded = false;
			if (f.placeholder && a.trim(f.placeholder) != "" && (typeof (h.attr("src")) == "undefined" || a.trim(h.attr("src")) == "")) {
				h.attr("src", f.placeholder);
			}
			h.one("appear", function () {
				if (!this.loaded) {
					...
					a("<img />").bind("load", function () {
						h.hide().attr("src", h.attr(f.data_attribute))[f.effect](f.effect_speed);
						i.loaded = true;
						var m = a.grep(c, function (n) {
							return !n.loaded;
						});
						c = a(m);
						if (f.load) {
						...
						var l = h.attr("lazygroup");
						if (typeof (l) != "undefined") {
							l = a.trim(l);
							if (l != "" && -1 == a.inArray(l, d)) {
								d.push(l);
								c.each(function (n, o) {
									if (a(o).attr("lazygroup") == l) {
										a(o).trigger("appear");
									}
								});
							}
						}
					}).attr("src", h.attr(f.data_attribute));
				}
			});
			if (0 !== f.event.indexOf("scroll")) {
				h.bind(f.event, function (j) {
					if (!i.loaded) {
						h.trigger("appear");
					}
				});
	...
	a.belowthefold = function (c, e) {
		var d;
		if (e.container === undefined || e.container === b) {
			d = $window.height() + $window.scrollTop();
		} else {
			d = $container.offset().top + $container.height();
		}
		return d <= a(c).offset().top - e.threshold;
	};
	a.rightoffold = function (c, e) {
		...
		if (e.container === undefined || e.container === b) {
			...
			d = $container.offset().left + $container.width();
		}
		return d <= a(c).offset().left - e.threshold;
	};
	a.abovethetop = function (c, e) {
		...
		return d >= a(c).offset().top + e.threshold + a(c).height();
	};
	a.leftofbegin = function (c, e) {
		...
		if (e.container === undefined || e.container === b) {
			...
			d = $container.offset().left;
		}
		return d >= a(c).offset().left + e.threshold + a(c).width();
	};
	a.inviewport = function (c, d) {
		return !a.rightofscreen(c, d) && !a.leftofscreen(c, d) && !a.belowthefold(c, d) && !a.abovethetop(c, d);
	};
